...
以此调试面板。此时，需要确保PyMiner的工作空间中有可以绘图的数据。
...
class BaseOperationDialog(QDialog):
    ...
    def get_help_content(self) -> str:
        ...
        if os.path.exists(self.help_file_path):
            with open(self.help_file_path, "rb") as f:
                ret = f.read()
                return ret.decode(chardet.detect(ret)["encoding"])
        return """# 帮助\n\n暂未定义帮助"""
    ...
    def no_var_in_workspace_hint(self) -> str:
        return "工作空间没有变量，请创建变量"
...
class DFOperationDialog(BaseOperationDialog):
    ...
    def __init__(self):
        ...
        self.button_preview = QPushButton(QCoreApplication.translate('BaseDFOperationDialog', "绘制"))
        self.button_store = QPushButton(QCoreApplication.translate('BaseDFOperationDialog', "编辑"))
