...
from .fastui.draw_boxplot import DrawBoxPlotDialog
from .fastui.functions import plt_show, plt_clf, EditTickDialog, EditLabelDialog, CreateSubplotDialog, CreateGridDialog
...
class PMDrawingsToolBar(PMGToolBar):
    ...
    def __init__(self):
        ...
        self.window_shown: Dict[str, QDialog] = {}
        self.dlgs: Dict[str, ClassVar[QDialog]] = {
            "draw_hist": DrawHistDialog,
            "draw_plot": PlotDialog,
            "draw_boxplot":DrawBoxPlotDialog,
            "edit_labels": EditLabelDialog,
            "edit_ticks": EditTickDialog,
            "create_subplot": CreateSubplotDialog,
            "create_grid": CreateGridDialog
        }
        ...
        self.add_tool_button("button_draw_hist", "直方图", "选择变量，绘制直方图",
                             create_icon(os.path.join(os.path.dirname(__file__), 'source/直方图.png')))

        self.add_tool_button("button_draw_plot", "折线图", "选择列表或数组变量，绘制折线图",
                             create_icon(os.path.join(os.path.dirname(__file__), 'source/折线图.png')))

        self.add_tool_button("button_draw_boxplot", "箱线图", "选择列表或数据集变量，绘制箱线图",
                             create_icon(os.path.join(os.path.dirname(__file__), 'source/箱线图.png')))

        self.addSeparator()
        self.add_tool_button("button_edit_labels", "标题与轴", "编辑标题及轴标签",
                             create_icon(os.path.join(os.path.dirname(__file__), 'source/label.png')))

        self.add_tool_button("button_edit_ticks", "轴刻度", "编辑坐标轴的刻度及刻度标签",
                             create_icon(os.path.join(os.path.dirname(__file__), 'source/ticks.png')))

        self.add_tool_button("button_create_grid", "网格线", "添加或删除坐标轴网格线",
                             create_icon(os.path.join(os.path.dirname(__file__), 'source/ticks.png')))

        self.add_tool_button("button_create_subplot", "创建子图", "在一个绘图页面中创建多个子图并进行分析",
                             create_icon(os.path.join(os.path.dirname(__file__), 'source/split.png')))

        self.add_tool_button("toolbutton_plt_show", "显示图像", "在外部窗口显示图像，并且编辑。",
                             create_icon(os.path.join(os.path.dirname(__file__), 'source/monitor.png')))

        self.add_tool_button("toolbutton_plt_clf", "清空图像", "清空当前图像",
                             create_icon(os.path.join(os.path.dirname(__file__), 'source/erase.png')))
    ...
    def on_button_clicked(self, button_name: str):
        name = button_name.lstrip("button_")
        if name in self.window_shown.keys():
            self.window_shown[name].show()
        else:
            self.window_shown[name] = self.dlgs[name]()
            self.window_shown[name].show()
    ...
    def bind_events(self):
        ...
        """
        self.get_control_widget("toolbutton_plt_show").clicked.connect(plt_show)
        self.get_control_widget("toolbutton_plt_clf").clicked.connect(lambda: plt_clf(self))

        for k in self._control_widget_dic.keys():
            if k.startswith("button"):
                def wrapper(val):
                    def callback():
                        self.on_button_clicked(val)

                    return callback

                cw = self.get_control_widget(k)
                if hasattr(cw, "clicked"):
                    cw.clicked.connect(wrapper(k))
