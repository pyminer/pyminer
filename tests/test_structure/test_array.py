...
def test_1d_normal():
    ...
    # 获取一维数组的显示格式
    assert ar.get_matrix() == [[-1, 10, 52, 13, 562, -122]]
    assert ar.get_header_name(0) == ['0', '1', '2', '3', '4', '5']
    assert ar.get_header_name(1) == ['0']
...
def test_2d_normal():
    ...
    assert ar.get_matrix() == [[1, 2, 3], [6, 5, 4], [-1.1, -2, -3], [-8, -7, -6]]
    assert ar.get_header_name(0) == ['0', '1', '2', '3']
    assert ar.get_header_name(1) == ['0', '1', '2']
...
def test_get_header_name():
    ...
    ar = ArrayAdapter(numpy.ones((3, 4, 5)).cumsum().reshape((3, 4, 5)))
    assert ar.get_header_name(2) == ['0', '1', '2', '3', '4']
