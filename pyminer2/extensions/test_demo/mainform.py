...
"""
This file pretends to be the main program of pyminer2, which implements
...
class Extension:
    def __init__(self, info):
        ...
        for subwindow_info in self.subwindows_info:
            ...
            self.subwindows_modules[module_name] = self.import_module(
                module_name)
    ...
    def start_subwindow(self, subwindow_class: str, config: dict) -> SubWindow:
        for subwindow_info in self.subwindows_info:
            ...
            if subwindow_class_name == subwindow_class:
                subwindow_cls = getattr(
                    self.subwindows_modules[module_name],
                    subwindow_class_name)
...
class MainForm:
    ...
    def load_extension(self, extension_package: str):
        package_path = f'pyminer2/extensions/test_demo/extensions/{extension_package}'
        ...
        # allow main program to update menu and subwindows to GUI after there
        # are created
        extension.start_menu = self.start_menu(extension.start_menu)
        extension.start_subwindow = self.start_subwindow(
            extension.start_subwindow)
        # allow main program to do something before start the extension and
        # after it ends
        extension.start = self.start_extension(extension)
        extension.end = self.end_extension(extension)
        # allow the extension to search dependencies and obtain their
        # interfaces
