...
from pyminer2.share.threads import *
from pyminer2.extensions.package_manager import Ui_Form as EnvManager_Ui_Form
from pyminer2.extensions.package_manager.package_install import Ui_Form as PackageInstall_Ui_Form
from pyminer2.extensions.package_manager.package_update import Ui_Form as PackageUpdate_Ui_Form
from pyminer2.extensions.package_manager import Ui_Form as PackageRemove_Ui_Form
from pyminer2.extensions.package_manager.package_setting import Ui_Form as PackageSetting_Ui_Form
from pyminer2.ui.source.qss.qss_tools import QssTools
...
class PackageManagerForm(QWidget, EnvManager_Ui_Form):
    ...
    def __init__(self, parent=None):
        ...
        self.pushButton_import.clicked.connect(
            self.pip_requirements_import)  # 从requirements.txt导入包
        self.pushButton_export.clicked.connect(
            self.pip_freeze_export)  # 导出当前包到requirements.txt
        ...
        self.comboBox.currentIndexChanged.connect(
            self.package_filter_change)  # 当下拉索引发生改变时发射信号触发绑定的事件
    ...
    def pip_requirements_import(self):
        ...
        import_path = QFileDialog.getOpenFileName(
            self, '导入requirements.txt', '', "文本文件(*.txt)")
    ...
    def package_filter_default(self):
        ...
        self.tableWidget.setEditTriggers(
            QAbstractItemView.NoEditTriggers)  # 将表格变为禁止编辑
    ...
    def package_filter_update(self):
        ...
        self.tableWidget.setHorizontalHeaderLabels(
            ['包名', '当前版本', ' 最新版本', '类型', '', ''])  # 设置表头
        # self.tableWidget.horizontalHeader().setSectionResizeMode(5,
        # QHeaderView.Stretch)  # 设置第五列宽度自动调整，充满屏幕
        self.tableWidget.horizontalHeader().setStretchLastSection(True)  # 设置最后一列拉伸至最大
        self.tableWidget.sortItems(0, Qt.AscendingOrder)
        self.tableWidget.setColumnHidden(3, False)  # 将第4列隐藏
...
class PackageInstallForm(QWidget, PackageInstall_Ui_Form):
    ...
    def pip_init(self):
        ...
        self.lineEdit_source.setStyleSheet(
    ...
    def dir_change(self):
        ...
        if self.comboBox_dir.currentText() == "默认位置":
            ...
            self.lineEdit_dir.setStyleSheet(
                "background-color:rgb(110, 190, 10);")
