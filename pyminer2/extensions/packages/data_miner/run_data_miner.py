...
# from data_miner2.extensions.extensionlib import BaseExtension, BaseInterface
...
from data_miner.share import threads  # 导入多线程处理模块
from data_miner.features.sample import io  # 导入数据相关操作模块
from data_miner.features.visualize import base as plot  # 导入可视化相关操作模块
from data_miner.features.statistics import basic_stats  # 导入统计相关操作模块
from data_miner.features.modelling import base as model  # 导入模型相关操作模块
from data_miner.features.report import pyreport  # 导入输出报告模块
from data_miner.features.preprocess import preprocess  # 导入数据预处理模块

# 导入UI相关模块
from data_miner.ui.base.mainForm import Ui_MainWindow
from data_miner.ui.base.newItem import Ui_Form as New_Ui_Form
from data_miner.ui.base.aboutMe import Ui_Form as AboutMe_Ui_Form
from data_miner.ui.base.option import Ui_Form as Option_Ui_Form
from data_miner.ui.source.qss.qss_tools import QssTools
import qdarkstyle
from pmgwidgets import get_style_sheet

__Author__ = """
By: data_miner Development Team
QQ: 454017698
Email: aboutlong@qq.com
"""
__Copyright__ = 'Copyright (c) 2020 data_miner Development Team'
__Version__ = '1.0.1'

root_dir = os.path.dirname(os.path.abspath(__file__)) + r'\data_miner'
...
class MyMainForm(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.slot_flush_console('info', 'system', 'data_miner已准备就绪')

        # 任务栏图标设置
        self.tray_icon = QSystemTrayIcon(self)
        self.tray_icon.setToolTip("data_miner数据分析")
        self.tray_icon.setIcon(QIcon(root_dir + '/ui/source/icons/logo.png'))

        title_action = QAction("data_miner数据分析", self)
    ...
    def closeEvent(self, event):
        ...
        if reply == QMessageBox.Ok:
            event.accept()
            # os.system(r"taskkill /F /IM data_miner.exe")  # 结束进程
        else:
            event.ignore()
...
class OptionForm(QWidget, Option_Ui_Form):
    ...
    def __init__(self, parent=None):
        ...
        self.setupUi(self)
        self.center()
        # QssTools.set_qss_to_obj(root_dir + "/ui/source/qss/data_miner.qss", self)
...
if __name__ == '__main__':
    ...
    # 从pmgwidgets提供的接口方法中获取样式表。但是在调试程序的时候可以不使用。
    ...
    # 窗口美化设置
    # QssTools.set_qss_to_obj(root_dir + "/ui/source/qss/data_miner.qss", app)
    # 设置窗口风格
