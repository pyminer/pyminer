...
class PMProcess():
    def __init__(self, args: List[str]):
        ...
        self.to = threading.Thread(target=self.enqueue_stream, args=(self.process.stdout, self.q, 1))
        self.te = threading.Thread(target=self.enqueue_stream, args=(self.process.stderr, self.q, 2))
        ...
        while True:
            if not self.q.empty():
                ...
                if idleLoops >= 5:
                    idleLoops = 0
                    print('write!!')
                    self.process.stdin.write('messsage\n'.encode('ascii'))# 模拟输入
