...
from PyQt5.QtCore import QFile, QFileInfo, QIODevice, QRegExp, QTextStream, Qt, QModelIndex
from PyQt5.QtWidgets import QAction, QApplication, QFileDialog, QMainWindow, QTextEdit, QTabWidget, \
    QMessageBox, QPushButton, QMenu, QToolBar, QToolButton, QListWidget, QListWidgetItem
from PyQt5.QtGui import QFont, QIcon, QColor, QSyntaxHighlighter, QTextCharFormat, QTextCursor, QCursor, \
    QKeyEvent, QPixmap, QMouseEvent
from typing import List, Tuple
from pyminer_new.extensions.extensionlib.pmext import PluginInterface
from pyminer_new.ui.generalwidgets import PMPushButtonPane,PMToolBar
...
class SyntaxChecker():
    '''
    SyntaxChecker是一个调用了jedi的类，负责对嵌入式ide提供智能提示等种种帮助。

    '''
    def __init__(self):
        self.reloaded_name_set = {'tensorflow', 'numpy', 'PyQt5', 'PySide2'}
        self.script: jedi.Script = None

    def reset(self):
        import importlib
        importlib.reload(module='jedi')

    def load_module(self, modules: List):
        jedi.preload_module(*modules)

    def update_script(self, script_str: str):
        '''
        设置jedi的当前脚本。
        :param script_str:
        :return:
        '''
        self.script = jedi.Script(script_str)

    def make_completion(self, line: int, col: int) -> List[jedi.api.classes.Completion]:
        '''
        获取自动补全的内容
        :param line: 光标所在行数（注意，这个是从1开始的，总之很坑就对了。）
        :param col: 光标所在列数（从0开始）
        :return:
        '''
        try:
            return self.script.complete(line, col)
        except:
            return []
...
class PMCodeEdit(QTextEdit):
    ...
    def _make_autocomp(self):
        ...
        self.syntax_checker.update_script(self.toPlainText())

        print('text', self.toPlainText(), 'pos', pos)
        completions = self.syntax_checker.make_completion(pos[0] + 1, pos[1])
        l = []
        ...
        print(completions)
        # self.popup_hint_widget

    def _get_textcursor_pos(self) -> Tuple[int, int]:
        print('长度', self.textCursor().position())
        print('行号', self.textCursor().blockNumber())
        print('列号', self.textCursor().columnNumber())
...
# from pyminer_new.ui.base.widgets.menu_tool_stat_bars import PMModernToolbar
...
np.d
    ...
    jedi.preload_module('numpy', 'PyQt5.QtWidgets')
    source = jedi.Script(source)
    t1 = time.time()
    l = source.complete(6, len('np.d'))
    print(l)
    t2 = time.time()
    print(t2 - t1)
