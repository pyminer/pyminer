from PyQt5.QtCore import QLineF, QPointF, Qt, QRectF, pyqtSignal, QObject
...
class PMGSimpleSignal():
    """
    简单的信号类
    此信号类接口类似于pyqtSignal，但父类不一定必须继承QObject。可以从其他地方发出，并且起到去耦合的作用。
    """
...
class CustomMidPoint(QGraphicsObject):
    ...
    signal_double_clicked = pyqtSignal(object)
...
class CustomPort(QGraphicsObject):
    ...
    def __init__(self, port_id: str, text: str = 'port', content: object = None, port_type='input'):
    ...
    def on_delete(self):
        print(self.connected_lines)
        for line in self.connected_lines:
            print('line_delete',line)
            line.on_delete()
