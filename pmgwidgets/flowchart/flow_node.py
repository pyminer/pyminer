...
from PyQt5.QtCore import QObject, QPointF, QSize, Qt
...
if TYPE_CHECKING:
    from pmgwidgets.flowchart.flowchart_widget import PMGraphicsScene, PMFlowWidget
from pmgwidgets.flowchart.flow_content import FlowContentEditableFunction
...
class Node(QObject):
    ...
    def __init__(self, canvas: 'PMGraphicsScene', node_id, text: str = '', input_ports: List[CustomPort] = None,
                 output_ports: List[CustomPort] = None, icon_path=r''):
    ...
    def on_edit_properties_requested(self):
        ...
        views += [('line_edit', 'text', 'Node Text', self.text),
        ]
        if self.content.ports_changable[0]:
            views.append(('entry_list', 'inputs', 'Set Inputs', [input_ids, input_texts], new_id_input))
        if self.content.ports_changable[1]:
            views.append(('entry_list', 'outputs', 'Set Outputs', [output_ids, output_texts], new_id_output))
        sp.set_items(views)
